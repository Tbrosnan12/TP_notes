...
\begin{itemize}
    ...
    \begin{align*}
         ...
         \end{pmatrix}u_L = 0  
     \end{align*} 
     Both of these have the same solutions $\xi_{+}= \begin{pmatrix}
         1 \\ 0
     \end{pmatrix}$ and $\xi_{-}= \begin{pmatrix}
         0 \\ 1
     \end{pmatrix}$. For $u_R$, $\xi_+$ has eigenvalue $+E$ and $\xi_-$ has eigenvalue $-E$ in the case of $u_L$ it is the other way round. It is convenient (see section 4.8 of my QFT notes) to normalize these spinors by:
     \begin{align*}
...
We can then use Feynman rules for QED (see section 9.2 of my QFT notes). These tell us to add to our Matrix element a $-ie\gamma^{\mu}$ for every vertex, note that in our case since we have split up the 
\end{itemize}
