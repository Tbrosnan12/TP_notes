...
\begin{itemize}
  \item Let us n8
  ''''             //////////////                                           ow investigate if there is a Noether current and a corresponding conserved charge. Recall the Dirac Lagrangian from \ref{D_act} is $\mathcal{L}_D = i\bar{\psi}\gamma^{\mu}\partial_{\mu}\psi-m\bar{\psi}\psi$. We can can clearly see that since this is quadratic in the spinors there is a symmetry in that we can define $\psi \rightarrow e^{i\alpha}\psi$ and $\psi^{\dagger}\rightarrow e^{-i\alpha}\psi^{\dagger}$. Infinitesimally these are: $\psi \rightarrow \psi +i\alpha\psi$ and $\psi^{\dagger} \rightarrow \psi^{\dagger} -i\alpha\psi^{\dagger}$. We can then use \ref{Noether} to write down that the conserved current (up to a sign is):
  \begin{flalign}
...
\begin{itemize}
  ...
  \begin{align*}
    L_{\text{Max}} &= -\frac{1}{4}\left[\partial^{\mu}A^{\nu}-\partial^{\nu}A^{\mu}\right]\left[\partial_{\mu}A_{\nu}-\partial_{\nu}A_{\mu}\right] \\
...
\begin{itemize}
  \item We can now expand $A^{\mu}(x)$ in terms of its Fourier transform. But we should first remember that our choice of constraints and gauges have made it so that $A^0=0$ so we are not interested in that term. The expansion is then:
    ...
    \begin{align*}
     ...
     \end{align*} 
     Thus to be most general we can pick $\boldsymbol{\xi}$ to be a linear combination of two orthonormal polarization vectors $\boldsymbol{\epsilon}_{r},~r=1,2$ which are orthogonal to $\textbf{p}$ so $\boldsymbol{\epsilon}_{r}\cdot \textbf{p}=0$. Them being orthonormal means $\boldsymbol{\epsilon}_{r}\cdot\boldsymbol{\epsilon}_{r'} = \delta_{rr'}$. With these vectors and the fact that the components of $\textbf{A}$ satisfy the KG equation we can write down the mode expansion of $\textbf{A}$:
     \begin{flalign*}
       \textbf{A} &= \int \frac{d^3p}{(2\pi)^3}\frac{1}{\sqrt{2|\textbf{p}|}}\sum_{r=1}^2\boldsymbol{\epsilon}_{r}(\textbf{p})\left[a^{r}_{\textbf{p}}e^{i\textbf{p}\cdot\textbf{x}}+a^{r\dagger}_{\textbf{p}}e^{-i\textbf{p}\cdot\textbf{x}}\right] \\
       \textbf{E}  &= \int \frac{d^3p}{(2\pi)^3}(-i)\sqrt{\frac{|\textbf{p}|}{2}}\sum_{r=1}^2\boldsymbol{\epsilon}_{r}(\textbf{p})\left[a^{r}_{\textbf{p}}e^{i\textbf{p}\cdot\textbf{x}}+a^{r\dagger}_{\textbf{p}}e^{-i\textbf{p}\cdot\textbf{x}}\right]
     \end{flalign*}
     ...
     \begin{align*}
        \sum_r\epsilon_r^i(p)\epsilon_r^j(p) = A \delta^{ij}+ B p^ip^j
      \end{align*} 
      Based on the two properties that these vectors satisfy, these are the only possible tensors. We can now impose the condition that $\boldsymbol{\epsilon}_{r}\cdot\boldsymbol{\epsilon}_{r'} = \delta_{rr'}$. If we multiply both sides by $\epsilon_s^i$ and sum over $i$ we get the following:
      \begin{align*}
        \sum_i\epsilon_s^i(p)\epsilon_s^i(p)\epsilon_s^j(p) = \sum_i\left[A\delta^{ij}\epsilon_s^i(p)+Bp_ip_j\epsilon_s^i(p)\right] \\
        \implies \epsilon_s^j(p) = A\epsilon_s^j(p) + 0 ~~~\implies A =1
      \end{align*}
      Where we have used the fact that $ \sum_i\epsilon_s^i(p)\epsilon_s^i(p)=1$ and $\sum_ip_i\epsilon_s^i(p) = 0 $. We can then repeat this by multiplying by $p_i$ and summing over $i$:
      \begin{align*}
        \sum_r\sum_ip^i\epsilon_s^i(p)\epsilon_s^j(p) &= p^j +Bp^j|\textbf{p}|^2 \\
         \implies B &= -\frac{1}{|\textbf{p}|^2}
      \end{align*}
      This gives the nice expression that:
  \begin{flalign*}
    \sum_r\epsilon_r^i(p)\epsilon_r^j(p) =  \delta^{ij}-\frac{p^ip^j}{|\textbf{p}|^2}
  \end{flalign*}
  We can now easily use this to check what relation the creation and annihilation operators satisfy:
  \begin{align*}
    [A^i(x),E^j(y)] =  \int \frac{d^3pd^3q}{(2\pi)^6}\sqrt{\frac{|\textbf{q}|}{4\textbf{p}|}}\sum_{r,s}\boldsymbol{\epsilon}^i_{r}(\textbf{p})\boldsymbol{\epsilon}^j_{s}(\textbf{q})&\left[[a^r_{\textbf{p}},a^{s}_{\textbf{q}}]e^{i(\textbf{p}\cdot\textbf{x}+\textbf{y}\cdot \textbf{q})}+[a^{r\dagger}_{\textbf{p}},a^{s}_{\textbf{q}}]e^{-i(\textbf{p}\cdot\textbf{x}-\textbf{y}\cdot \textbf{q})} \right.\\
    + &\left.[a^r_{\textbf{p}},a^{s\dagger}_{\textbf{q}}]e^{i(\textbf{p}\cdot\textbf{x}-\textbf{y}\cdot \textbf{q})}+[a^{r\dagger}_{\textbf{p}},a^{s\dagger}_{\textbf{q}}]e^{-i(\textbf{p}\cdot\textbf{x}+\textbf{y}\cdot \textbf{q})} \right] \\
     = \int \frac{d^3pd^3q}{(2\pi)^6}\sqrt{\frac{|\textbf{q}|}{4\textbf{p}|}}\sum_{r,s}\boldsymbol{\epsilon}^i_{r}(\textbf{p})\boldsymbol{\epsilon}^j_{s}(\textbf{q})&\left[[a^r_{\textbf{p}},a^{s}_{\textbf{q}}]e^{i(\textbf{p}\cdot\textbf{x}+\textbf{y}\cdot \textbf{q})}+[a^{r\dagger}_{\textbf{p}},a^{s}_{\textbf{q}}]e^{-i(\textbf{p}\cdot\textbf{x}-\textbf{y}\cdot \textbf{q})}\right.\\
    + &\left.[a^r_{\textbf{p}},a^{s\dagger}_{\textbf{q}}]e^{i(\textbf{p}\cdot\textbf{x}-\textbf{y}\cdot \textbf{q})}+[a^{r\dagger}_{\textbf{p}},a^{s\dagger}_{\textbf{q}}]e^{-i(\textbf{p}\cdot\textbf{x}+\textbf{y}\cdot \textbf{q})}\right]
  \end{align*}
...
\begin{itemize}
  ...
  \begin{align*}
    \bra{0}T\{A_i(\textbf{x})A_j(\textbf{y})\}\ket{0} &=   \bra{0}T\left\{\int \frac{d^3pd^3q}{(2\pi)^6}\frac{1}{\sqrt{4|\textbf{p}||\textbf{q}|}}\sum_{r,s}\boldsymbol{\epsilon}^i_{r}(\textbf{p})\boldsymbol{\epsilon}^j_{s}(\textbf{q})\left[a^{r}_{\textbf{p}}e^{i\textbf{p}\cdot\textbf{x}}+a^{r\dagger}_{\textbf{p}}e^{-i\textbf{p}\cdot\textbf{x}}\right] \right.\\
     &~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\left.\times\left[a^{s}_{\textbf{q}}e^{i\textbf{q}\cdot\textbf{y}}+a^{s\dagger}_{\textbf{q}}e^{-i\textbf{q}\cdot\textbf{y}}\right] \right\}\ket{0} \\
...
\begin{itemize}
  ...
  \end{align*}
  Where $\psi^+$ and $\psi^-$ are the negative and positive frequency components of the fields and $S_F(x-y)$ is the Dirac Feynman operator defined in \ref{T_dirac}. With this definition through the anti-commutator we can see that $\wick{\c{\psi(x)} \c{\psi(y)}} = 0$ and $\wick{\c{\bar{\psi}(x)} \c{\bar{\psi}(y)}}$. With these definitions Wick's theorem (seee section\ref{sub:wick_s_theorem}) take pretty much the same form. The only thing we need to be wary of is that contractions of fermions that are not beside each other will pick up a minus sign as we need to make them be beside each other for the contraction. 
