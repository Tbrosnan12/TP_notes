...
\begin{itemize}
  ...
  \begin{equation*}
  \begin{split}
  \bra{x} e^{-i\sqrt{\textbf{P}jj,/^2 + m^2}(t-t')} &\ket{x'}  = \int_{0}^{\infty} \frac{p^2dp}{(2\pi)^2}  \int_{-1}^{1} d \eta e^{-i\sqrt{p^2 + m^2}(t-t')+ ip|\textbf{x}-\textbf{x}'|\eta} \\
...
\int_{-\infty}^{\infty}&h(x)e^{Af(x)}dx =  \int_{-\infty}^{\infty}h(x)e^{A\left[f(x_0) + \frac{1}{2}f''(x_0)(x-x_0)^2+\mathcal{O}((x-x_0)^3)\right]}dx \\ 
 \approx   h(x_0)e^{Af(x_0)}&\int_{-\infty}^{\infty}e^{-A\frac{1}{2}|f''(x_0)|(x-x_0)^2}dx = \sqrt{\frac{2\pi}{A|f''(x_0)|}}h(x_0)e^{Af(x_0)} 
  ...
  \item Back to the relevant integral \ref{Rel_propagator}, we can see that for us $f(p) = -i\sqrt{p^2 + m^2}(t-t')+ip|\textbf{x}-\textbf{x}'|$ and $h(p) = p$. Samson then uses this as an argument to get the form of the integral, but there is a small flaw in this argument, in that $f(p)$ is \textbf{imaginary} so the exponential doesn't behave quite like is outlined in the Laplace integral approximation. Instead we can think of this as such; we have the an exponential $e^{i g(p)}$ for some real function $g(p)$. This function rotates around the unit circle , so as we integrate over it. This is also influenced by the fact that we wish to look at the region out side the light cone, i.e. where $|\textbf{x}-\textbf{x}'| >> (t-t')$. Then we have that the exponent $g(p)$ is varying quite rapidly as $|\textbf{x}-\textbf{x}'|$ is large for any substantial time frame. This means the integrating, i.e the summing over rapidly oscillating exponential, leads to adding up all the points around the unit circle, which add up to 0. The only place where this does not happen is where $g(p)$ is stationary! We can find this stationary point $p_0$ of $g(p)= -\sqrt{p^2 + m^2}(t-t')+p|\textbf{x=}-\textbf{x}'|$ via $g'(p_0)=0$, so: 
  \begin{equation*}
  \begin{split}
      g'(p_0) = \frac{-p_0(t-t')}{\sqrt{p_0^2+m^2}}+|\textbf{x}-\textbf{x}'| =0 \implies p_0^2 = \frac{m|\textbf{x}-\textbf{x}'|}{(t-t')}
      \end{split}
    \end{equation*}  
    \item So 
\end{itemize}
