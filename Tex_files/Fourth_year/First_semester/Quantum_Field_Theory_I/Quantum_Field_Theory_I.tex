...
\begin{itemize}
  \item This section is following Dirac's small book ``Lectures on Quantum mechanics'' 
\end{itemize}
...
\begin{itemize}
  ...
  \begin{align*}
    H'(p,q) = H + \lambda^{m}\phi_m = p_iq^i-L + \lambda^{m}\phi_m  
  \end{align*}
...
\begin{itemize}
  ...
  \begin{flalign*}
    \frac{df}{dt} = \{f,H\}
  \end{flalign*}
  If we then return to our constrained Hamiltonian we will have that 
  \begin{align*}
   ...
   \end{align*} 
   Where we have essentially assumed that $\lambda^{m}$ are not functions of $p_i$ and $q_i$, though Dirac does not impose this and still somehow ends up at roughly these equations. What this essentially means is that the time derivative of a function $g(p,q)$ now becomes:
