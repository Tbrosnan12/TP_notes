...
\begin{itemize}
  ...
  \begin{flalign}
    ...
    \end{align*}
  This actually means these equations are not independent as they imply each other (through the relation $(\sigma^{\mu}p_{\mu})(\bar{\sigma}^{\nu}p_{\nu}) = p_0^2-p_{i}p_j\sigma^{i}\sigma^{j} = p_{0}^2-p_ip_j\delta^{ij} = m^2$). \footnote{This is totally wrong and needs to be fixed at some point... }This means the components must be linear dependent, i.e. the same up to a factor. This means we can make the ansatz $u_1 = (\sigma^{\mu}p_{\mu})\xi$, then $u_2 = m\xi$. But we can scale both $u_1$ and $u_2$ by any factor we want, so we choose the most symmetric one by multiplying by $1/\sqrt{p^{\mu}\sigma_{\mu}}$ leaving us with the neat solution:
  \begin{flalign}
       u(p) = \begin{pmatrix}
...
\begin{itemize}
  ...
  \end{align}
  Here upper and lower index's are only to indicate that we are summing over the variables, we operate in a euclidean metric so that all upper index's equal lower. Note that the coefficients $\lambda_{m}$ can also be any sort of functions of the $q's$ and $p's$.  
\end{itemize}
...
\item In the standard case we can then take the coefficients of $\delta p_i$ and $\delta q^i$ to separately be zero, but in the case of a constrained system $p_i$ and $q^i$ are no longer independent, being related by at least one $\phi^{m}(p,q)$. To fix this we need some way of ensuring that the $p$'s and $q$'s stay on the surface $\phi^{m}(p,q)=0$ in the phase space. Since $\phi_m=0$, this means the variations of $p$ and $q$ must satisfy:
\begin{align*}
...
\begin{itemize}
 ...
 \begin{align*}
     ...
     \begin{align*}
        \{R,\phi_m\}  = r^n\phi_n
     \end{align*}
     It can also be shown that the Poisson bracket of two first class functions, is also first class. Looking at our total Hamiltonian \ref{H_tot}, we can see that the number arbitrary coefficients $v_a$ is equal to the number of independent primary first class constraints. This is because there is a $v^a$ for each $V^m_a$, and there is a $V^m_a$ for each $\phi_m$ such that $\{\phi_m,\phi_n\}=0$ for all $n$. 
\end{itemize}
...
\begin{itemize}
   \item It can be checked that $\{\pi_0,\partial_i\pi_i \} \approx 0$, meaning both these constraints are first class. Now this is not ideal as we can recall from section \ref{sub:first_second_class_variables}, that the number of first class constraints is the same as the number of arbitrary coefficients in our total Hamiltonian \ref{H_tot} . To fix this we can introduce an equal number of gauge fixes that will act as constraints. Introducing new constraints, if chosen right, will change some of the first class constraints to second class, as the old constraints may not have $0$ Poisson bracket with the new constraints.  
   ...
   \begin{align*}
    ...
    \item With this we have found $2$ new constraints, the same number as arbitrary coefficients in our total Hamiltonian due to the presence of $2$ first class constraints $\pi_0\approx 0$ and $\partial^i\pi_i \approx 0$. Together the constraints are:
    \begin{align}  
...
\begin{itemize}
  \item The next step is to calculate the constraint matrix \ref{constraint matrix}. We do this with the hopes that it will allow us to construct the Dirac bracket for this theory. We have four constraints as seen in \ref{constraints_gauge}, this means $C_{ab} = \{\phi_a,\phi_b\}$ will be a $4 \times 4$ matrix. We will order the constraints by $\pi_0,\partial^i\pi_i,A^0, \partial_iA^i \approx 0$. We can easily see that $C_{ii}=0$ as the constraints as $\{f,f\} =0$ by definition. We are left to check the remaining $12$ coefficients, but we only need to check $6$ as this matrix is by definition anti-symmetric. 
...
\subsection{Trace Technology} % (fold)
\label{sub:trace_technology}
Through out many calculations in QED the traces of various combinations of gamma matrices defined in section \ref{sub:dirac_matrices}. We proved in section \ref{sub:dirac_matrices} that the gamma matrices are trace-less. This can be further extended to all odd combinations of gamma matrices, by using $\gamma^{5}$ instead of $\gamma^{\mu}$ in that proof, as $\{\gamma^5,\gamma^{\mu}\} = 0 $ \footnote{This can be shown from the alternate definition of $\gamma^5$ as $\gamma^5 = -\frac{i}{4!}\epsilon^{\mu\nu\rho\sigma}\gamma_{\mu}\gamma_{\nu}\gamma_{\rho}\gamma_{s}$.}. Hence:
\begin{align*}
  \text{tr}(\underbrace{\gamma^{\nu} \cdot \cdot \cdot \gamma^{\mu}}_{\text{n=odd}}) =0 
\end{align*}
For the even number of gamma matrices we can use their definition \ref{gamma_def} to manipulate and calculate their traces. For example:
\begin{align*}
  \text{tr}(\gamma^\mu\gamma^\nu) &= \text{tr}(2\eta^{\mu\nu}\cdot\mathbbm{1}-\gamma^\nu\gamma^\mu) = 8\eta^{\mu\nu}-\text{tr}(\gamma^{\mu}\gamma^{\nu}) \\
  & \implies \text{tr}(\gamma^\mu\gamma^\nu) = 4\eta^{\mu\nu}
\end{align*}
Where we have to remember that the trace is over the spinor indices not the $\mu,\nu$ indices. Similarly one can show that: 
\begin{align*}
...
+ \eta^{\mu\sigma} \eta^{\nu\rho} \right).
\end{align*}    
% subsection trace_technology (end)
