...
\item Proceeding with these commutation relations however will not fix the negative energy problem as we still end up with a minus sign from \ref{v(p)} (See calculation above). We can however fix this, if we relabel $c^{r}_{\textbf{p}} -> c^{r\dagger}_{\textbf{p}}$ and $c^{r\dagger}_{\textbf{p}} -> c^{r}_{\textbf{p}}$. We are free to do this now as the anti-commutation relations are symmetric for these operators.  This means $c^{r\dagger}_{\textbf{p}}c^{r}_{\textbf{p}} = - c^{r}_{\textbf{p}}c^{r\dagger}_{\textbf{p}}  +\text{constant}$.This means the Hamiltonian \ref{Ham_D} nicely becomes: 
 \begin{flalign}
...
\begin{itemize}
  \item Recall from classical mechanics that we had the notion of the \emph{Poisson bracket} defined for two functions $f(p,q)$ and $g(p,q)$ as:
  \begin{flalign}
  \label{poisson}
  ...
  \begin{flalign*}
    \frac{df}{dt} = \{f,H\}
  \end{flalign*}
  If we then return to our constrained Hamiltonian \ref{Ham-const} we can see that taking the variantion we get:
\begin{align*}
  \delta H' &= \delta p_i\dot{q}^i+p_i\delta \dot{q}^i-\delta L + \lambda^{m}\delta \phi_m =  \frac{\partial H}{\partial q^i}\delta q^i+\frac{\partial H}{\partial p_i}\delta p_i \lambda^{m}\left[\frac{\partial \phi_m}{\partial q^i}\delta q^i+\frac{\partial \phi_m}{\partial p^i}\delta p^i\right] \\
  & = \left(\frac{\partial H}{\partial q^{i}}+\lambda^{m}\frac{\partial \phi_m}{\partial q^i}\right)\delta q^{i}+\left(\frac{\partial H}{\partial p_i}+\lambda^{m}\frac{\partial \phi_m}{\partial p_i}\right)\delta p_{i}\\
  & =
\end{align*}
Where we have used the fact that $H =H(q,p) \implies \delta H = \frac{\partial H}{\partial q^i}\delta q^i+\frac{\partial H}{\partial p_i}\delta p_i$ and $\phi =\phi(p,q) \implies \delta \phi_m = \frac{\partial \phi_m}{\partial q^i}\delta q^i+\frac{\partial \phi_m}{\partial p^i}\delta p^i$

  \begin{align*}
     & \frac{dq_i}{dt} = \frac{\partial H}{\partial p_i} + \lambda^{m}\frac{\partial \phi_m}{\partial p_i}\\
     & \frac{dp_i}{dt} = -\frac{\partial H}{\partial q_i} - \lambda^{m}\frac{\partial \phi_m}{\partial q_i}
   \end{align*} 
   Notice the $\lambda^{m}$ are still functions of $p$ or $q$, but their derivatives will always be multiplying $\phi_{m}$ which will vanish.  What this essentially means is that the time derivative of a function $g(p,q)$ now becomes:
   \begin{flalign}
   \label{dgdt}
     \frac{d g}{dt} = \{g,H\} +\lambda^{m}\{g,\phi_m\}
   \end{flalign}
   This can be show to be the same as $\{g,H+\lambda^{m}\phi_m\}$, even for $\lambda^m$ as functions of $p$ and $q$ as: $\{g,\lambda^{m}\phi_m\} = \{g,\lambda^m\}\phi_m + \lambda^m\{g,\phi_m\}$ and since $\phi_m =0$ we can ignore this first term. With this we can say that $\dot{g} = \{g,H_T\}$ where $H_T = H+\lambda^{m}\phi_m$ is called the \emph{total Hamiltonian}. 
\end{itemize}
   \subsubsection{Weak and strong equations}
   \begin{itemize}
     ...
     \begin{align*}
        \phi_m \approx 0 
      \end{align*} 
      And call these \emph{weak equations}. Any other regular equality is called a \emph{strong equation}. 
   \end{itemize}
   \subsubsection{Secondary constraints}
   \begin{itemize}
     \item Since \ref{dgdt} holds for all $g(p,q)$ we can apply it for one of the constraints $\phi_n$. We should have that the time derivative of our constraints are $\dot{\phi}=0$ as it does not make much sense to talk about constraints that change. This means we have:
     \begin{align*}
       \{\phi_n,H\} +\lambda^{m}\{\phi_n,\phi_m\} =0 
     \end{align*}
     If we call $N_{ab} = \{\phi^{a},\phi^b\}$, then there are three cases:
     \begin{itemize}
        \item $N$-invertible, which means  
      \end{itemize} 
   \end{itemize}
