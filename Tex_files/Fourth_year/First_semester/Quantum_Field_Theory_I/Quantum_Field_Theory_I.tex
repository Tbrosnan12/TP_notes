...
\begin{tikzpicture}[scale=2]
  ...
  \end{pgfonlayer}
\end{tikzpicture}
      \end{array} + \hspace{1mm} \cdot \cdot \cdot \right)}{(2\pi)^(4)\delta^{(4)}(0)}
  \end{align*}
...
\begin{itemize}
  \item The general picture for calculating the cross section is to consider a wave packet of initial states and evolve them for a large time using $e^{-Ht}$. We then overlap the result with a set of final wave-packets representing the desired final state particles $\ket{\phi}$. This gives us to the probability amplitude for the production of final state, which we will be able to relate to the cross section. 
  ...
  \begin{align*}
   ...
   \end{align*}
   Due to the fact that $\ket{\textbf{k}} = \sqrt{2E_{\textbf{k}}}a^{\dagger}_{\textbf{k}}\ket{0}$. 
\end{itemize}
