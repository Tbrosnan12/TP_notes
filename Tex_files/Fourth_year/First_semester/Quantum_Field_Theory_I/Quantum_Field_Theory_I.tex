...
\begin{itemize}
  ...
  \begin{align}
     ...
     \label{L_half}
     & \Lambda_{\frac{1}{2}} = \exp(\frac{1}{2}i\Omega_{\mu\nu}S^{\mu\nu})
   \end{align} 
...
\begin{itemize}
  \item We would now naturally like to find out what are the invariants we can make out of the spinors $\psi$ which transform according to \ref{S_transform}. Let us first try what usually works, that being the standard combination $\psi \psi^{\dagger}$.  
\end{itemize}
\end{document}