...
\end{align}
    ...
    Which means
     \begin{align*}
        \pi_{\mu} &= \frac{\partial \mathcal{L}}{\partial \dot{A}^{\mu}} = \frac{\partial }{\partial \dot{A}^{\mu}}\left(-\frac{1}{4}\eta_{\mu\delta}F^{\delta\gamma}\eta_{\gamma\nu}F^{\mu\nu}\right) \\
...
\begin{align*}
  \{\pi_0,H\} & = - \int d^3z\frac{\partial H(\textbf{x})}{\partial A^\mu(\textbf{z})}\frac{\partial \pi_0(\textbf{x})}{\partial \pi_\mu(\textbf{z})} = - \int d^3z\frac{\partial H(\textbf{x})}{\partial A^0(\textbf{z})}\frac{\partial \pi_0(\textbf{x})}{\partial \pi_0(\textbf{z})}   \\
  & =- \int d^3z\frac{\partial (A^0\partial_iE^i)}{\partial A^0(\textbf{z})}\delta(\textbf{x}-\textbf{z})= - \int d^3z\delta(\textbf{x}-\textbf{z})\partial_iE^i = 0 ,  \\ 
  & \implies \partial_iE^i = 0, ~~~\implies \partial_i\pi_i = 0
\end{align*}
...
\begin{itemize}
   ...
   \begin{align*}
     \frac{\partial\left(\partial_iA^i(\textbf{x})\right)}{\partial A^j(\texbf{z})} &= -\int d^3x'\frac{\partial A^i(\textbf{x}')}{\partial A^j(\textbf{x})}\frac{\partial}{\partial (x')^i}\delta(\textbf{x}-\textbf{x}') = -\int d^3x\delta^{i}_j\delta(\textbf{x}'-\textbf{z})\frac{\partial}{\partial (x')^i}\delta(\textbf{x}-\textbf{x}') \\
     & = \frac{\partial}{\partial z^j}\delta(\textbf{x}-\textbf{z})
   \end{align*}

   :
   \begin{align*}
    \partial_0(\partial_iA^i)  = \{\partial_iA^i,H\} 
   \end{align*}
\end{itemize}
